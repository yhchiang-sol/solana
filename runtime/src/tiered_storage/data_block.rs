...
enum AccountBlockEncoder {
    Raw(Cursor<Vec<u8>>),
    Lz4(lz4::Encoder<Vec<u8>>),
}

pub struct AccountBlockWriter {
    len: usize,
    encoder: AccountBlockEncoder,
}
...
impl AccountBlockWriter {
        ...
        Self {
            ...
            encoder: match encoding {
                AccountBlockFormat::AlignedRaw => {
                    AccountBlockEncoder::Raw(Cursor::new(Vec::new()))
                }
                AccountBlockFormat::Lz4 => AccountBlockEncoder::Lz4(
    ...
    pub fn write(&mut self, buf: &[u8], len: usize) -> Result<(), Error> {
        let result = match &mut self.encoder {
            AccountBlockEncoder::Raw(cursor) => cursor.write_all(&buf[0..len]),
            AccountBlockEncoder::Lz4(lz4_encoder) => lz4_encoder.write_all(&buf[0..len]),
        };
        if result.is_ok() {
            self.len += len;
        }
    ...
    pub fn finish(self) -> Result<(Vec<u8>, usize), Error> {
        match self.encoder {
            AccountBlockEncoder::Raw(cursor) => Ok((cursor.into_inner(), self.len)),
            AccountBlockEncoder::Lz4(lz4_encoder) => {
...
impl AccountBlock {
    pub fn decode(encoding: AccountBlockFormat, input: &[u8]) -> Result<Vec<u8>, Error> {
        match encoding {
            ...
            AccountBlockFormat::AlignedRaw => panic!("Not implemented"),
        }
    }
}
...
/*
   ...
   fn test_compress_and_decompress() {
       ...
       {
           let ads = TieredStorage::new_for_test(&path.path, true);
